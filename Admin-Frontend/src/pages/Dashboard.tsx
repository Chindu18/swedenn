...
useEffect(() => {
  const fetchMovie = async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        setmovie(lastMovie);
        console.log("Fetched movie:", lastMovie); // ✅ log directly
      }
    } catch (err) {
      console.error(err);
    }
  ...
  // Modal state for pending → paid toggle
  ...
  // Fetch bookings and seats
 useEffect(() => {
  if (!movie.title) return; // wait until movie is loaded
  ...
  const fetchData = async () => {
    try {
      ...
      // Combine all bookings for table
      ...
      console.log("Pending Money:", pendingResp.data.totalAmount);
      console.log("Paid Money:", paidResp.data.totalAmount);
    } catch (error) {
  ...
  fetchData();
}, [movie]); // ✅ Run whenever movie changes


  // Filter bookings based on payment status
  useEffect(() => {
    if (paymentStatus.toLowerCase() === "pending") {
      setBookings(allBookings.filter(b => b.paymentStatus.toLowerCase() === "pending"));
    } else if (paymentStatus.toLowerCase() === "paid") {
      setBookings(allBookings.filter(b => b.paymentStatus.toLowerCase() === "paid"));
    } else {
      setBookings(allBookings);
    }
  }, [paymentStatus, allBookings]);
          ...
          <CardContent className="p-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-6">
              <div className="flex-1">
                ...
                <input type="text" placeholder="Enter movie name" value={movie.title} className="w-full border rounded-lg px-3 py-2 bg-gray-100 cursor-not-allowed" readOnly />
              </div>

              <div className="flex-1">
                <label className="block text-sm font-medium text-gray-700 mb-1">Search by Booking ID</label>
                <input type="text" placeholder="Enter booking ID..." className="w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-primary outline-none" />
              </div>
          ...
          <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
            <div className="bg-white p-6 rounded-lg shadow-lg text-center">
              ...
              <div className="flex justify-center gap-4">
                <button onClick={confirmToggleStatus} className="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700">
                  Yes
                </button>
                <button onClick={cancelToggleStatus} className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">
                  No
                </button>
              </div>
            </div>
